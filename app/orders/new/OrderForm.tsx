...
// ── DoorItem ──────────────────────────────────────────────────────────────────
// Memoized so that sibling-door or top-level state changes don't re-render it.
...
interface DoorItemProps {
  ...
  onOpenStockPicker: (index: number) => void
  onImageChange: (index: number, file: File | null) => void
}

const DoorItem = memo(function DoorItem({
  item, index, canRemove, onUpdate, onRemove, onOpenStockPicker, onImageChange,
}: DoorItemProps) {
  return (
    <div className="rounded-lg border border-gray-100 bg-gray-50 p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-gray-700">{index + 1}. Kapı</span>
          {item.stock_model_id ? (
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div>
          ...
          <select
            ...
            onChange={e => onUpdate(index, 'door_type', e.target.value)}
          ...
          {item.door_type === 'diger' && (
            <input
              ...
              onChange={e => onUpdate(index, 'door_type_custom', e.target.value)}
        ...
        <div>
          ...
          <select
            ...
            onChange={e => onUpdate(index, 'measurement_preset', e.target.value)}
            className="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
            <option value="">— Seçiniz —</option>
        ...
        <div>
          ...
          <div className="mt-1 flex gap-1.5">
            <input
              ...
              onChange={e => onUpdate(index, 'right_opening_count', Number(e.target.value))}
              placeholder="Sağ"
            ...
            <input
              ...
              onChange={e => onUpdate(index, 'left_opening_count', Number(e.target.value))}
              placeholder="Sol"
              className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
        ...
        <div>
          ...
          <div className="mt-1 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-900">
            {fmtTRY.format(Number(item.quantity) * Number(item.unit_price))}
          </div>
        </div>
        <div>
          ...
          <select
            ...
            onChange={e => onUpdate(index, 'lock_brand', e.target.value)}
            className="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
            <option value="">— Seçiniz —</option>
          ...
          {item.lock_brand === 'DİĞER' && (
            <input
              ...
              onChange={e => onUpdate(index, 'lock_brand_custom', e.target.value)}
        ...
        <div>
          ...
          <select
            ...
            onChange={e => onUpdate(index, 'lock_system', e.target.value)}
            className="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
            <option value="">— Seçiniz —</option>
          ...
          {item.lock_system === 'DİĞER' && (
            <input
              ...
              onChange={e => onUpdate(index, 'lock_system_custom', e.target.value)}
        ...
        <div>
          ...
          <select
            ...
            onChange={e => onUpdate(index, 'frame_color', e.target.value)}
            className="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
            <option value="">— Seçiniz —</option>
          ...
          {item.frame_color === 'Diğer' && (
            <input
              ...
              onChange={e => onUpdate(index, 'frame_color_custom', e.target.value)}
        ...
        <div>
          ...
          <select
            ...
            onChange={e => onUpdate(index, 'mdf_thickness', e.target.value)}
            className="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
            <option value="">— Seçiniz —</option>
        ...
        <div>
          ...
          <input
            ...
            onChange={e => onUpdate(index, 'sac_kalinligi', e.target.value)}
            className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={e => onUpdate(index, 'kanat_kalinligi', e.target.value)}
            className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
...
export default function OrderForm({ companyId, userId, orderId, initialData, cariler = [], initialCariId, userRole, staffList = [] }: Props) {
  ...
  const [stockPickTarget, setStockPickTarget] = useState(0)
  ...
  // ── Stable callbacks ─────────────────────────────────────────────────────────
  // useCallback with [] deps — functional setItems updates mean no closure over stale state.

  const updateItem = useCallback((index: number, field: string, value: string | number) => {
    setItems(prev => prev.map((it, i) => {
      if (i !== index) return it          // unchanged items keep the same reference → memo bails out
      ...
      if (field === 'measurement_preset') {
        next.measurement = value === 'standart' ? '90x200x20' : ''
      }
      return next
    }))
  }, [])
  ...
  const handleImageChange = useCallback((index: number, file: File | null) => {
    setItems(prev => prev.map((it, i) => i === index ? { ...it, image_file: file } : it))
  }, [])

  // Ref keeps stockModelsRef.current fresh on every render without adding it as a dep.
  const stockModelsRef = useRef<StockModel[] | null>(null)
  stockModelsRef.current = stockModels

  const openStockPickerFor = useCallback(async (itemIdx: number) => {
    setStockPickTarget(itemIdx)
    setStockOpen(true)
    if (stockModelsRef.current !== null && stockModelsRef.current.length > 0) return
  ...
  const kdvRate = useMemo(
    () => Number(kdvRateSelect === 'diger' ? kdvRateCustom : kdvRateSelect) || 0,
    [kdvRateSelect, kdvRateCustom]
  )
  const kdvTutari   = useMemo(() => itemsTotal * kdvRate / 100, [itemsTotal, kdvRate])
  const genelToplam = useMemo(() => itemsTotal + kdvTutari,     [itemsTotal, kdvTutari])

  const selectedCari = useMemo(
    () => localCariler.find(c => c.id === cariId),
    [localCariler, cariId]
  )
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (!isEdit && cariId) {
      const selectedCariForSubmit = localCariler.find(c => c.id === cariId)
      if (selectedCariForSubmit) {
        customerName  = selectedCariForSubmit.contact_name || selectedCariForSubmit.name
        customerPhone = selectedCariForSubmit.phone ?? ''
        customerCity  = selectedCariForSubmit.city  ?? ''
      }
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <fieldset className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                id="customer_phone" name="customer_phone" type="text" required
            ...
            <div>
              ...
              <input
                id="customer_city" name="customer_city" type="text" required
                value={form.customer_city} onChange={handleChange}
                className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
          </div>
      ...
      <fieldset className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <legend className="mb-4 text-sm font-semibold text-gray-700">Kapılar</legend>
          <div className="space-y-5">
            {items.map((item, idx) => (
              <DoorItem
                ...
                onUpdate={updateItem}
                onRemove={removeItem}
                onOpenStockPicker={openStockPickerFor}
          ...
          <div className="mt-5 border-t border-gray-100 pt-4">
            ...
            <input
              id="production_duration_weeks" name="production_duration_weeks"
              type="number" min={1} max={52} required
              value={form.production_duration_weeks} onChange={handleChange}
              className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
      ...
      {isEdit && (
        <fieldset className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="production_start_date" className="block text-sm font-medium text-gray-700">Üretime Başlama Tarihi</label>
              <input id="production_start_date" name="production_start_date" type="date" value={form.production_start_date} onChange={handleChange} className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div>
              <label htmlFor="ready_date" className="block text-sm font-medium text-gray-700">Hazır Tarihi</label>
              <input id="ready_date" name="ready_date" type="date" value={form.ready_date} onChange={handleChange} className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div>
              <label htmlFor="shipped_date" className="block text-sm font-medium text-gray-700">Sevk Tarihi</label>
              <input id="shipped_date" name="shipped_date" type="date" value={form.shipped_date} onChange={handleChange} className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div>
              <label htmlFor="delivered_date" className="block text-sm font-medium text-gray-700">Teslim Tarihi</label>
              <input id="delivered_date" name="delivered_date" type="date" value={form.delivered_date} onChange={handleChange} className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div className="sm:col-span-2">
              <label htmlFor="operation_note" className="block text-sm font-medium text-gray-700">Operasyon Notu</label>
              <textarea
                id="operation_note" name="operation_note" rows={3}
                value={form.operation_note} onChange={handleChange}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {stockModels.map(m => {
                    ...
                    return (
                      ...
                      >
                        {m.image_url && (
                          <img src={m.image_url} alt={m.name} className="mb-2 h-28 w-full rounded-md object-contain bg-gray-50" />
                        )}
                        <p className="font-bold text-sm text-gray-900">{m.name}</p>
                        {m.door_type && <p className="text-xs text-blue-600 mt-0.5">{m.door_type}</p>}
                        {m.measurement && <p className="text-xs text-gray-400 mt-0.5">{m.measurement}</p>}
